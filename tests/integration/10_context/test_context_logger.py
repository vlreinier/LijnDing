...
async def test_context_logger(backend):
    ...
    try:
        ...
        @stage(
            name="test_logger_stage",
            backend=backend,
            workers=2 if backend == "process" else 1,
        )
        def logger_stage_for_backend(context: Context, item: str):
            context.logger.info("processing_item", item_id=item)
            return item
        ...
        item_logs = [
            ...
            if log.get("event") == "processing_item"
            and log.get("logger") == "lijnding.stage.test_logger_stage"
        ]

        # The process backend does not share memory and its logs are not captured by the handler.
        if backend != "process":
            assert len(item_logs) == 2, (
                f"Expected 2 item logs, but found {len(item_logs)}"
            )
        ...
        stream_started_log = next(
            (log for log in parsed_logs if log.get("event") == "stream_started"), None
        )
        stream_finished_log = next(
            (log for log in parsed_logs if log.get("event") == "stream_finished"), None
        )
