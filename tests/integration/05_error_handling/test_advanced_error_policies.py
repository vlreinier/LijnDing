...
def test_route_to_pipeline():
    ...
    failing_stage = stage(StatefulFailer(fail_on_items=[123]), error_policy=ErrorPolicy(
        mode="route_to_pipeline",
        route_to_pipeline=stage(collect_to_dead_letter)
    ))
...
def test_transform_and_retry_succeeds():
    ...
    failing_stage = stage(process_string, error_policy=ErrorPolicy(
        mode="route_to_pipeline_and_retry",
        route_to_pipeline=transformer,
        retries=1
    ))
...
def test_transform_and_retry_fails():
    ...
    failing_stage = stage(Failer(message="always fails"), error_policy=ErrorPolicy(
        mode="route_to_pipeline_and_retry",
...
async def test_async_route_to_pipeline():
    ...
    failing_stage = stage(StatefulFailer(fail_on_items=[123]), backend="async", error_policy=ErrorPolicy(
        mode="route_to_pipeline",
